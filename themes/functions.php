...
function get_debug() {
  ...
  if(isset($ma->config['debug']['maveric']) && $ma->config['debug']['maveric']) {
    $html .= "<hr><h3>Debuginformation</h3><p>The content of CMaveric:</p><pre>" . htmlent(print_r($ma, true)) . "</pre>";
  }    
  if(isset($ma->config['debug']['session']) && $ma->config['debug']['session']) {
    $html .= "<hr><h3>SESSION</h3><p>The content of CMaveric->session:</p><pre>" . htmlent(print_r($ma->session, true)) . "</pre>";
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CMaveric::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function base_url($url=null) {
  return CMaveric::Instance()->request->base_url . trim($url, '/');
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CMaveric::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CMaveric::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CMaveric::Instance()->themeParentUrl . "/{$url}");
}
...
function current_url() {
  return CMaveric::Instance()->request->current_url;
}
...
function render_views($region='default') {
  return CMaveric::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CMaveric::Instance()->views->RegionHasView(func_get_args());
}
