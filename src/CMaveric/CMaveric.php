...
class CMaveric implements ISingleton {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath   = MAVERIC_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = MAVERIC_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl  = $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $ma->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $ma
    ...
    $ma = &$this;
    // First the default Maveric themes/functions.php
    include(MAVERIC_INSTALL_PATH . '/themes/functions.php');
  ...
  /**
   * Draw HTML for a menu defined in $ma->config['menus'].
   *
